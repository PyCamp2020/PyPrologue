...
class Process_nonBasic_Station():
    sheet_name = "临安溪口"
    data_cols = {"station": "C", "y/m/d_start": "D", "y/m/d_end": "E", "rainfall": "F"}
    ...
    def get_station_Len_List(self):
        col = self.data_cols["station"]
        data = self.sheet.range(col+"1:"+col+str(self.nrows)).value
    ...
    def get_rainfall(self):
        ...
        rainfall = self.sheet.range(col+"1:"+col+str(self.nrows)).value
        return rainfall
    ...
    def __reset_day_timeseries(self, start_time, end_time, rainfall):
        ...
        for start, end, value in zip(start_time, end_time, rainfall):
            ...
            average_rain = round(float(value)/time_len, 3)  # 运算结果保留3位小数
            count = 1
            while count <= time_len and (start+count) < 24:  # 这里注意
                timeseries[start + count] = average_rain
                count += 1
...
# folder = r"C:\\Users\\gr\\Desktop\\临安太湖源项目\\里畈等摘录表\\里畈等摘录表\\"
# path = folder + "市岭溪口降水量摘录表.xls"
# result = r"C:\Users\gr\Desktop"
...
class process_rainstation_data():
    ...
    sheet_name = "sheet1"
    year, stationCord, stationID = 2011, 0, ""
    ...
    def __read_excel(self):
        ...
        # sheet = excel.sheets["Sheet1"]
        sheet = excel.sheets[self.sheet_name]

        ID = sheet.range("A1").value.split()
        # self.stationID = ID[1][:-7]
        self.stationID = ID[1]
        self.year = int(sheet.range("D2").value)
        self.stationCord = sheet.range("H2").value
        ...
        page1 = sheet.range("A6:G55").value
        page2 = sheet.range("H6:N55").value
        page3 = sheet.range("O6:U55").value

        page4 = sheet.range("A61:G110").value
        page5 = sheet.range("H61:N110").value
        page6 = sheet.range("O61:U110").value

        page7 = sheet.range("A116:G165").value
        page8 = sheet.range("H116:N165").value
        page9 = sheet.range("O116:U165").value

        page10 = sheet.range("A171:G220").value
        page11 = sheet.range("H171:N220").value
        page12 = sheet.range("O171:U220").value

        page13 = sheet.range("A226:G275").value
        page14 = sheet.range("H226:N275").value
        page15 = sheet.range("O226:U275").value

        page16 = sheet.range("A281:G330").value
        page17 = sheet.range("H281:N330").value
        page18 = sheet.range("O281:U330").value

        page19 = sheet.range("A336:G385").value
        page20 = sheet.range("H336:N385").value
        page21 = sheet.range("O336:U385").value

        page22 = sheet.range("A391:G411").value
        page23 = sheet.range("H391:N411").value
        page24 = sheet.range("O391:U410").value


        self.__data = [page1, page2, page3, page4, page5, page6,
    ...
    def __get_rainfall_timeseries(self):
        ...
        for page in self.__data:
            for line in page:
                ...
                value = line[4]
                if line[4] is None: value = 0.0
                # if line[4][-2] is ".":  value = line[4][:-2]
                # elif line[4][-1] is "*":  value = line[4][:-1]
    ...
    def __complete_hour(self):
        ...
        while index < count-1:
            ...
            if day != self.__rainfall_tseries[index+1][2]:  # 当前日的时间序列
                ...
                if start_h == rain_slice[0][4]: start_h -= 1
                num = 0
                ...
                for slices in rain_slice:  # 中间时间
                    ...
                    if end_h < start_h:
                        end_h = 24 + end_h
                    if end_h-start_h > 1:
                        dis, step = end_h - start_h, round(rain/(end_h - start_h), 3)
                        num = 1
                ...
                miss_hour = [hour for hour in range(0, 24) if hour not in exist_hour]
                for h in range(0, 24):  # 填充当日时间序列中缺失的时间
                    if h in miss_hour:
    ...
    def write_rainfall_tseries(self):
        if os.path.isfile(self.__txt_path):
            ...
            outfile = self.__txt_path + "\\" + str(self.year) + self.stationID + "坝上水位.txt"
        else:
...
class process_hydstation_data():
    ...
    def __read_excel(self):
        ...
        page22 = sheet.range("A391:G440").value
        page23 = sheet.range("H391:N440").value
        page24 = sheet.range("O391:U440").value

        page25 = sheet.range("A446:G495").value
        page26 = sheet.range("H391:N495").value
        page27 = sheet.range("O391:U495").value

        page28 = sheet.range("A501:G550").value
        page29 = sheet.range("H501:N520").value
        #page30 = sheet.range("O391:U495").value
    ...
    def __get_rainfall_timeseries(self):
        ...
        for page in self.__data:
            for line in page:
                ...
                arr3 = str(line[4]).split('.')
                if len(arr3) > 1:   wl_last = arr3[0]  # 水位的特殊记录方式
                wl = float(wl_last + "." + arr3[0])
                #q = line[5]
                if line[5] is None: v = 0.0
                else : v = float(line[5])

                if line[6] is None: q = 0.0
                else : q = float(line[6])
                ...
                if len(self.__timeseries)>0 and hour == self.__timeseries[-1][3] \
                    ...
                    self.__timeseries[-1][5] = (self.__timeseries[-1][5] + v) / 2
                    self.__timeseries[-1][6] = (self.__timeseries[-1][6] + q) / 2
                else:
                    self.__timeseries.append([self.year, month, day, hour, wl, v, q])
    ...
    def __complete_hour(self):
        ...
        while index < count-1:
            ...
            if day != self.__timeseries[index+1][2]:  # 当前日的时间序列
                ...
                for h in range(0, 24):  # 填充当日时间序列中缺失的时间
                    if h in miss_hour:
                        num = [exist_hour.index(hour) for hour in range(0, h) if hour in exist_hour]
                        if len(num) > 0:  # 向前查找
                            wl, v, q = rain_slice[num[-1]][-3], rain_slice[num[-1]][-2], rain_slice[num[-1]][-1]
                            new_hour_rain.append([self.year, month, day, h, wl, v, q])
                            continue
                        num = [exist_hour.index(hour) for hour in range(h + 1, 24) if hour in exist_hour]
                        # if len(num) > 0:  # 向后查找
                        #     wl, v, q = rain_slice[num[0]][-3], rain_slice[num[0]][-2], rain_slice[num[0]][-1]
                        #     new_hour_rain.append([self.year, month, day, h, wl, v, q])
                        #     continue
                        new_hour_rain.append([self.year, month, day, h, 0.0, 0.0, 0.0])
                        continue
                    num = exist_hour.index(h)
        ...
        while index < count:
            ...
            for day in range(1, month_len + 1):
                if day in miss_day:
                    ...
                    if len(num) > 0:  # 向前查找
                        wl, v, q = rain_slice[num[-1] + 23][-3], rain_slice[num[-1] + 23][-2], rain_slice[num[-1] + 23][-1]
                    else:  # 向后查找
                        num = [exist_day.index(day) for day in range(day + 1, month_len + 1) if day in exist_day]
                        wl, v, q = rain_slice[num[0] + 23][-3], rain_slice[num[0] + 23][-2], rain_slice[num[0] + 23][-1]
                    for h in range(0, 24):  # 补全每天时间
                        new_day_rain.append([self.year, month, day, h, wl, v, q])
                    continue
                num = exist_day.index(day)
                for h in range(24):
                    new_day_rain.append(rain_slice[num + h])
    ...
    def write_hyd_tseries(self):
        ...
        for data in self.__timeseries:
            line = "%d/%d/%d\t%d:00:00\t%f\t%f\t%f\n" % (data[0], data[1], data[2], data[3], data[4], data[5], data[6])
            fl.write(line)
        fl.close()


folder = r"C:\\Users\\gr\\Desktop\\临安太湖源项目\\里畈等摘录表\\里畈等摘录表\\"
path = folder + "里畈水库2010水库水文要素摘录表.xls"
result = r"C:\Users\gr\Desktop"
# excel_path = r"F:\抚州项目(验收)\项目建模数据\AAA 戎丹雅工作交接\2 降雨流量数据处理" \
#              r"\0、原始数据\16年基本站资料\62405800 南城\62405800-2016年南城站洪水水位摘录表.xls"
# txt_path = "C:\\Users\gr\Desktop"
rs = process_hydstation_data(path, result)
